...
class MD5:
    ...
    def __hash(self, message):
        ...
        for chunk in chunks:
            ...
            for i in range(4):
                for j in range(4):
                    a = r(
                        funcs[i], a, b, c, d, words[self.indices[i][j * 4]],
                        self.Rs[i][0], self.T[i][j * 4]
                    )
                    d = r(
                        funcs[i], d, a, b, c, words[self.indices[i][j * 4 + 1]],
                        self.Rs[i][1], self.T[i][j * 4 + 1]
                    )
                    c = r(
                        funcs[i], c, d, a, b, words[self.indices[i][j * 4 + 2]],
                        self.Rs[i][2], self.T[i][j * 4 + 2]
                    )
                    b = r(
                        funcs[i], b, c, d, a, words[self.indices[i][j * 4 + 3]],
                        self.Rs[i][3], self.T[i][j * 4 + 3]
                    )
    ...
    def __to_binary_string(self, string):
        ...
        return ''.join(
            "{:08b}".format(byte) for byte in bytearray(string.encode('utf-8'))
        )
